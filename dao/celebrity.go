...
func SelectCelebrityById(Id int, mid int) (model.Celebrity, error) {
	var celebrity model.Celebrity

	row := dB.QueryRow("SELECT id, name, info, synopsis, url, award, urlinfo1, urlinfo2, urlinfo3, nameinfo1, nameinfo2, nameinfo3, yearinfo FROM celebrity WHERE id = ? and movieId = ?", Id, mid)
	if row.Err() != nil {
